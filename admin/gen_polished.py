...
list_template = """import { useEffect } from "react";
...
form_template = """import React, { useState, useEffect } from "react";
...
for entity, fields in entities.items():
    ...
    # Fix for fetchCategorys pluralization mismatch
    fetch_name = f"fetch{Name}s"
    if entity == "category": fetch_name = "fetchCategorys"

    feat_dir = BASE_DIR / entity / "pages"
    feat_dir.mkdir(parents=True, exist_ok=True)
    
    # List Page
    list_content = list_template.replace("fetchREPLACE_NAMEs", fetch_name).replace("REPLACE_NAME", Name).replace("REPLACE_ENTITY", entity).replace("REPLACE_PLURAL", plural).replace("REPLACE_ROUTE", route)
    ...
    input_fields = ""
    has_textarea = "description" in fields
    for f in fields:
        if f == "description":
    ...
    textarea_import = 'import { Textarea } from "@/shared/components/ui/textarea";' if has_textarea else ""
    
    # Add/Edit Pages
    common_form = form_template.replace("REPLACE_NAME", Name).replace("REPLACE_ENTITY", entity).replace("REPLACE_API_ROUTE", plural).replace("REPLACE_ROUTE", route).replace("REPLACE_FIELDS_INIT", fields_init).replace("REPLACE_INPUT_FIELDS", input_fields).replace("REPLACE_TEXTAREA_IMPORT", textarea_import)
    
    with open(feat_dir / f"Add{Name}.tsx", "w", encoding="utf-8") as f:
        f.write(common_form.replace("REPLACE_ACTION", "Add"))
        
    with open(feat_dir / f"Edit{Name}.tsx", "w", encoding="utf-8") as f:
        f.write(common_form.replace("REPLACE_ACTION", "Edit"))

print("Generated polished Admin components.")
