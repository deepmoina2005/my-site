...
const AllServices = () => {
  ...
  const handleDelete = (id: string) => {
    if (confirm("Are you sure?")) {
      dispatch(deleteService(id)).then(() => toast.success("Deleted"));
    }
  };
  ...
  return (
    <div className="p-6">
      <CardHeader className="flex flex-row items-center justify-between px-0">
        ...
        <Button onClick={() => navigate("/services/add")}><Plus className="mr-2 h-4 w-4" /> Add Service</Button>
      </CardHeader>
            ...
            {loading ? (
              ...
              <TableRow key={item._id}>
                ...
                <TableCell className="text-right space-x-2">
                  <Button variant="ghost" size="icon" onClick={() => navigate(`/services/${item._id}/edit`)}>
