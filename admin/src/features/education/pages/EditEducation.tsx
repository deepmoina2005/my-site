...
import { updateEducation } from "../educationSlice";
...
const EditEducation = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dispatch(updateEducation({ id: id!, data: formData })).unwrap();
      toast.success("Education entry updated successfully");
      navigate("/education/all");
    } catch (error: any) {
      toast.error(error || "Failed to update education entry");
    } finally {
      setSaving(false);
    }
  };
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button type="submit" className="w-full h-12 text-lg" disabled={saving}>
              {saving ? "Updating..." : "Update Education Entry"}
            </Button>
          </form>
        </CardContent>
      </Card>
